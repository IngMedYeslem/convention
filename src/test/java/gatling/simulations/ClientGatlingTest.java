...
public class ClientGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all clients").get("/api/clients").headers(headersHttpAuthenticated).check(status().is(200)))
                    ...
                    http("Create new client")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"emailClient\": \"SAMPLE_TEXT\"" +
                                ", \"whatsClient\": \"SAMPLE_TEXT\"" +
                                ", \"obsClient\": null" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created client").get("${new_client_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created client").delete("${new_client_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
