 ...
 */
public class FactureGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all factures").get("/api/factures").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new facture")
                        .post("/api/factures")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"numFacture\": 0" +
                                ", \"dateFacture\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"montantTotal\": 0" +
                                ", \"montantTTC\": 0" +
                                ", \"tva\": 0" +
                                ", \"observations\": null" +
                                ", \"ancienneRef\": \"SAMPLE_TEXT\"" +
                                ", \"typeFacture\": \"NORMALE\"" +
                                ", \"statut\": \"BROUILLON\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_facture_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created facture").get("${new_facture_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created facture").delete("${new_facture_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Facture entity").exec(scn);
