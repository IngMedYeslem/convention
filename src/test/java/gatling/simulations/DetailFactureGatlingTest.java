...
public class DetailFactureGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all detailFactures").get("/api/detail-factures").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new detailFacture")
                        .post("/api/detail-factures")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"montantTVA\": 0" +
                                ", \"montantTTC\": 0" +
                                ", \"observations\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_detailFacture_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created detailFacture").get("${new_detailFacture_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created detailFacture").delete("${new_detailFacture_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DetailFacture entity").exec(scn);
