...
type RestOf<T extends IFacture | NewFacture> = Omit<T, 'dateFacture' | 'dateEcheance' | 'dateCreation'> & {
...
export class FactureService {
  ...
  protected convertDateFromClient<T extends IFacture | NewFacture | PartialUpdateFacture>(facture: T): RestOf<T> {
    return {
      ...
      dateCreation: facture.dateCreation?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFacture: RestFacture): IFacture {
    return {
      ...
      dateCreation: restFacture.dateCreation ? dayjs(restFacture.dateCreation) : undefined,
    };
  }
